...
export default function PublicDiary() {
  ...
  if (!diaryData) {
    return <div className="min-h-screen flex items-center justify-center">Diary not found</div>;
  }
      ...
      <div className="max-w-2xl mx-auto">
        <div className="card p-8 text-center">
          ...
          {diaryData.description && (
            <p className="text-secondary-600 mb-8">{diaryData.description}</p>
          )}

          {checkData?.isOwner ? (
            <p className="text-secondary-600">This is your diary. Visit your dashboard to view notes.</p>
          ) : checkData?.hasWritten ? (
            <p className="text-secondary-600">You've already written a farewell note. Thank you!</p>
          ) : (
            <button onClick={handleWriteNote} className="btn btn-primary flex items-center gap-2 mx-auto">
